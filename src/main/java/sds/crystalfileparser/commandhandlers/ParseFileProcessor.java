...
import sds.crystalfileparser.domain.events.RecordParsed;
import sds.crystalfileparser.domain.events.RecordParseFailed;
import sds.crystalfileparser.domain.events.FileParseFailed;
import sds.crystalfileparser.domain.events.FileParsed;
...
import sds.crystalfileparser.domain.core.Field;
    ...
    public void process(ParseFile message) {
        ...
        try {
            ...
            switch (FilenameUtils.getExtension(blob.getFileName()).toLowerCase()) {
                case "cif":
                    records = new CifParser(storage.getFileStream(new Guid(message.getBlobId()), message.getBucket()));
                    break;
                default:
                    ...
                    fileParseFailed.setMessage(String.format("Cannot parse file %s. Format is not supported.", blob.getFileName()));
            ...
            while (iterator.hasNext()) {
                try {
                    ...
                    storage.addFile(blobId, blobId.toString() + ".cif", record.getData().getBytes(), "chemical/x-cif", bucket, null);
                    ...
                    LOGGER.debug("Published event: " + recordParsed.toString());
